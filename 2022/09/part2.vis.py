...
def printSnake(positions):
    global locations
    grid = [['.' for _ in range(-15, 45)] for _ in range(-15, 45)]
    for location in locations:
        grid[location[1]+30][location[0]+30] = '#'
    for key in sorted(positions.keys()):
        pos = positions[key]
        if key == 0: key = 'H'
        grid[pos[1]+30][pos[0]+30] = key
    print('\033[H')
    for line in grid:
        for char in line:
            print(char, end='')
